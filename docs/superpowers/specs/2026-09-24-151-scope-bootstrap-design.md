...
Add a Bootstrap task state machine inside the #150 Rust Downlink worker. It has no separately running host loop and is not owned by `LiveSession`. The Downlink worker schedules at most one bootstrap HTTP request in flight across Scopes in the first version, independently of its ordinary delta request and live socket. Rotate requested Scopes after each page and release local transactions between pages. The Uplink worker remains independent. WebSocket and HTTP callbacks only enqueue tagged events and wake the Downlink worker; its pump owns application, pagination, retries and completion. Bootstrap may have a focused helper/module for its state transitions, but only the shared Downlink worker drives it. A blocked historical request must not hold the worker pump, a database transaction, or its command-serialization lock while awaiting the network.

Persisted requested/loading/catching-up tasks resume after client reopen and connection setup. Every historical request retains the same S from the subscription. A terminal response lost before local commit may observe a newer H on retry. Once its barrier is committed, reconnect and repeated calls retain it. Socket replacement alone does not cancel the persistent Bootstrap run; correlate its HTTP response independently of the socket epoch and validate the current subscription identity/run. A stale response after unsubscribe, retry, close/reopen or schema rebuild cannot recreate or complete a replaced task.
