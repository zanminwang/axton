...
The operation executor also runs inside the application's transaction, for both kinds and both delivery paths. It claims a canonical call before validating its arguments, so a retry returns that call's saved result even after a later batch. A fresh call reads changed records and Model outputs through loaders; an additional read-only Model output acquires its record stamp before loading. Its authority is returned at the caller's declared read version. A Query whose settlement carries changes or publications is rejected with `query.effects_forbidden` before any stamp, readback or publication ([Mutations and Queries](../../schema/actions.md#8-crosscutting-concepts)). A rejected call rolls back its own savepoint, while a persistence fault aborts the transaction. The legacy mutation path described in [Push](push.md) remains available.
