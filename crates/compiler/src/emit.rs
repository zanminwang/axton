...
fn ts_actions(v: &Value, o: &mut String) {
    ...
    o.push_str(" }\n}; }\n");
    o.push_str("/** Queries resolve with the backend result (direct); `once` reuses a saved complete result, `enqueue` accepts them durably and `invalidate` discards saved results. */\nexport function makeQueries(port:CallPort) { return {\n");
    for (name, version) in &queries {
        writeln!(o, " {}: (args:{name}Input, options?:{name}Options & OnceOptions):Promise<{name}Output> => port.invokeQuery('{name}',{version},encode{name}Input(args),decode{name}Output,options),", lower(name)).unwrap();
    }
    o.push_str(" enqueue: {\n");
    for (name, version) in &queries {
        durable(o, "  ", name, *version);
    ...
    o.push_str(" },\n invalidate: {\n");
    for (name, version) in &queries {
        writeln!(o, "  {}: (args:{name}Input):Promise<void> => port.invalidateQuery('{name}',{version},encode{name}Input(args)),", lower(name)).unwrap();
    }
    o.push_str(" }\n}; }\n");
}
...
/// The Dart named parameters carrying a direct Query's `once` and
/// `refresh` controls: their own names unless a business input or the store
/// selector already uses them.
fn dart_once_params(action: &Value) -> (String, String) {
    let mut taken: Vec<String> = arr(action, "inputs")
        .iter()
        .map(|i| s(i, "name").to_string())
        .collect();
    taken.push(dart_store_param(action));
...
pub fn client_typescript(v: &Value, runtime: &str) -> String {
    ...
    writeln!(o, "export {{ CallError, type Call, type CallOptions, type CallOutcome, type CallStatus, type OnceOptions, type ServerOptions, type Connection, type ConnectionOptions, type BootstrapPhase, type BootstrapStatus, type Subscription, type SubscriptionStatus }} from \"{runtime}\";").unwrap();
...
fn dart_action_runtime(v: &Value, o: &mut String) {
    ...
    for (class, doc, kind, direct, member) in [
        ...
        if class == "Queries" {
            o.push_str(" late final QueryInvalidations invalidate = QueryInvalidations(client);\n");
        }
        for (name, version) in &latest {
            let action = actions
                .iter()
            ...
            let method = if direct && kind == axton_core::CallKind::Query {
                let (once, refresh) = dart_once_params(action);
                write!(controls, ", bool {once} = false, bool {refresh} = false").unwrap();
                write!(forwarded, ", once: {once}, refresh: {refresh}").unwrap();
                "invokeQuery"
            } else if direct {
                "invokeDirectAction"
            } else {
            ...
            writeln!(o, " Future<{}> {}({}) => client.{method}<{output}>('{name}', {version}, {{{arguments}}}, {decoder}, {forwarded});", if direct { output.clone() } else { format!("Call<{output}>") }, lower(name), signature).unwrap();
        }
        o.push_str("}\n");
    }
    // `queries.invalidate` discards saved `once` results of one argument set.
    o.push_str("/// Discards the saved `once` results of one Query argument set, for every store policy.\nclass QueryInvalidations {\n final Client client; QueryInvalidations(this.client);\n");
    for (name, version) in &latest {
        let action = actions
            .iter()
            .find(|a| s(a, "name") == *name && a["version"].as_u64() == Some(*version))
            .unwrap();
        if crate::action_names::kind(action) != axton_core::CallKind::Query {
            continue;
        }
        let params = dart_action_params(action);
        let signature = if params.is_empty() {
        ...
        writeln!(
            o,
            " Future<void> {}({signature}) => client.invalidateQuery('{name}', {version}, {{{}}});",
            lower(name),
            dart_action_arguments(action)
        )
        .unwrap();
    }
    o.push_str("}\n");
