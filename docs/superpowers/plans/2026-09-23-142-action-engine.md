...
**Files:** Create crates/client/src/actions.rs; modify crates/client/src/{lib,ddl,queue,mutate,policies,push,schema_store}.rs; test crates/sqlite/tests/{push,settlement}.rs and new crates/sqlite/tests/actions.rs.
...
- [ ] Capture result values at that call's readback, before executing later calls. Aggregate only authority records at batch level; persist each individual response before saving the batch receipt. Reject call ID reused with different intent without replacing original data or saved result.
...
- [ ] Node/RN share observer logic where supported; Dart uses WeakReference and strong waiter state with equivalent semantics. Validate runtime feature support on supported RN/Hermes/toolchain; if a platform cannot support the mechanism, provide a documented bounded observer adapter without changing the user API, and test its actual lifecycle rather than assuming JS browser support.
