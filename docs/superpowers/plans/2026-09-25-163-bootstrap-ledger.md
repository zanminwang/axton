...
- Report defects via existing application `onError` callbacks with no repeated pump flood.
...
| `crates/sqlite/tests/bootstrap.rs` | Real SQLite row corruption, candidate isolation, deduplication and bind count |
...
- [ ] **Step 5: Run** `cargo test -p axton-sqlite --test bootstrap --locked`. Expected: all focused ledger tests pass, including >999 names and corrupt candidate isolation.
...
- [ ] **Step 1: Write worker tests** in `crates/sqlite/tests/bootstrap_worker.rs`. Inject one malformed active row and one healthy schedulable row. After `Start`/reopen and `Next`, assert one `LedgerIssue` names the bad channel and a healthy `Request` is still issued. Pump and wake repeatedly without changing the row: assert no more issue actions and no repeated zero-millisecond `Wait`. Change the malformed value and wake: assert one new issue. Repair or remove the row, wake, then corrupt it again: assert one new issue. Add a reached healthy barrier beside a bad active row on reopen and assert settlement still occurs.
...
- [ ] **Step 4: Add host dispatch tests** using the existing scripted action fixtures: JS `connection.test.mjs` should observe one `Error` at `onError` for a `ledgerIssue` action, and Dart `connection_test.dart` should observe one `StateError`. In `packages/client-js/connection.mts`, add the action to the typed `DownlinkAction` union and dispatch it to `options.onError?.(Error(action.message))`. In Dart, add the `ledgerIssue` switch case with `onError?.call(StateError(action['message'] as String))`. Preserve status projection: the action does not impersonate a Bootstrap transition.
...
Every target requirement maps to a task: active-row isolation (1), strict named reads (1), chunked and deduplicated candidate reads (2), corrupt candidate isolation (2), worker diagnostic visibility and duplicate suppression (3), host callbacks (3), and D10 regression checks (2 and final verification). The interfaces use `LedgerIssue` and `LedgerScan` consistently. No implementation, build, or test result is claimed by this planning document.
