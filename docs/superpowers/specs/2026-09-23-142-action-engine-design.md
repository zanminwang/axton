...
The canonical args object is the source of request inputs. Keep Model Action operands flat as approved in #141: create contains identity and fields, update contains identity and only supplied patch fields, and delete is the identity object. The shared Rust normalizer, durable intent, direct intent and backend handler use that same shape; identity/patch wrappers belong only to internal Model operation rows. Derived create/update/delete rows remain the source of local optimistic replay. Generate those rows once from the same validated args within the same transaction. Do not permit an input/operation mismatch to enter the queue. Optional Model operands normalize to null; ordinary nullable parameters are required keys with nullable values; omitted update patch fields remain omitted. Input arguments and read-contract declarations become immutable once a batch freezes. Persist the frozen result read-contract metadata locally as well: validate the returned snapshot against that original field contract before normalizing compatible additions for the current schema. This metadata contains no business results; a fresh result missing a field already declared by its frozen contract remains invalid.
...
claimCall inserts the row if absent, then SELECT FOR UPDATE locks it in the same transaction used by business writes. A concurrent duplicate waits and then observes the stored response. A null response is only an in-progress row inside that uncommitted transaction; normal execution must not commit a placeholder. saveCall writes the canonical complete response only for an uncompleted row created by the current application transaction, checked against owner/call identity and the full-width claim_tx. The explicit transaction ID remains valid across ambient savepoints; do not derive ownership from the tuple xmin or discard its transaction epoch. A fault rolls back the placeholder, business writes, stamps and publications together.

For a durable batch, retain the outer client claim/sequence validation and per-call savepoints. Claim each call before its business savepoint. If a saved response exists, replay it without handler or Loader execution. Otherwise run the handler and result processing; a per-call rejection rolls back that call's savepoint, then stores its failure outcome outside that savepoint but inside the batch transaction. Infrastructure transaction failure rolls back the whole delivery. Before converting handler/Loader exceptions into application failure data, recognize adapter retryable transaction faults (PostgreSQL 40001/40P01 and wrapped Prisma equivalents), propagate their original cause through the native host boundary, and let the existing whole-transaction runner retry. Never persist a serialization/deadlock fault as a terminal business outcome. This includes distinct call IDs concurrently changing the same business row, not only duplicate claim conflicts. Save the batch receipt and per-call outcomes before the outer commit. Reply and wake Channel listeners only after commit.
...
Generated handlers receive {ctx,args}. ctx contains the current application transaction, authenticated owner, stable call ID and the existing changes/publication collectors; the application supplies business logic and database access. Preserve one handler registration per Action version for both entry points. New public backend bindings expose ActionRejected for stable business rejection codes, retaining the existing rejection semantics. Generated args are the approved flattened Model inputs and ordinary values, not old internal data/patch wrappers.

Handler returns explicit ordinary values and Model identity objects. With no explicit output fields, a void/undefined handler return means an empty explicit output object; implicit Model outputs still resolve from inputs. Normalize this case before JSON transport. Missing fields on a declared explicit output remain errors. For relatedTodo Todo?, it returns {relatedTodo:{id:...}} or {relatedTodo:null}. The engine validates the object, extracts its identity and invokes the appropriate versioned Loader. Implicit create/update output identities come from their inputs; delete outputs are input identity confirmations and succeed only when authoritative readback confirms that identity is absent. A handler that leaves the deleted row present fails result processing and rolls back the call. The handler cannot replace an implicit identity. An absent optional mutation operand produces a null implicit result; a supplied create/update operand still requires a non-null loaded Model even when its declared cardinality is optional. Explicit nullable Model outputs may resolve a selected missing identity to null.

Required changed-record readback, stamps and publications retain the shared transaction path. Reuse a changed record's loaded snapshot when an output references that same identity/read version; additional read-only Model outputs acquire existing/initialized stamp evidence before the Loader read, holding that record metadata lock through commit so a concurrent mutation cannot pair old content with a newer stamp. EnsureStamp initializes absent evidence without advancing an existing business-change stamp. Preserve result ordering and multiplicity even if backend reads are deduplicated.

Assemble each full Model result from its canonical identity plus versioned normalized state; authority state currently excludes identity fields and cannot itself serve as the complete generated Model.
...
The Rust settlement/direct-application result includes transient completion events after its local transaction commits. Carry these events through SyncCycle, common/native bindings and every SDK transport consumer; diagnostic report extraction must not discard them. Emit terminal events for every removed queued Action, including unsent lifecycle dependents rejected transitively, using their own call IDs and rejection reasons. Explicit pending-work discard/rebuild must similarly terminate affected live observations without claiming unknown frozen work did not execute. TS/RN and Dart registries map callId to weak observation states. They do not persist the business result or reinterpret Rust settlement outcomes.

Application completion and diagnostic callbacks cannot replace an already committed Action outcome. Settle internal observations first, continue delivery after a callback throws, and report callback exceptions separately through the host runtime's uncaught-error mechanism. A callback exception does not trigger another invocation or become a transport failure.

Register a submitted call's observation state before emitting the work wakeup that can dispatch it, within the SDK's serialized submit completion. Do not leave an await gap in which a fast response can arrive before registration. If close races submission, close must either prevent submission or terminate its registered observation before returning; it must never return a permanently pending handle.
