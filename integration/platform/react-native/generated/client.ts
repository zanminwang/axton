...
export { ActionError, type ActionCall, type ActionOptions, type ActionOutcome, type ActionStatus, type ServerOptions, type Connection, type ConnectionOptions, type BootstrapPhase, type BootstrapStatus, type Subscription, type SubscriptionStatus } from "../../../../packages/client-react-native/index.ts";
import { schema, liveModels, makeActions, GeneratedTransaction, Mutate, type LiveModels } from "./generated.ts";
...
export class GeneratedClient {
 ...
 private constructor(client: Client, connection: Connection | undefined) { this.client = client; this.connection = connection; this.models = liveModels(client); this.actions = makeActions(client); this.mutate = new Mutate(client); this.scopes = new Scopes(client); this.channels = new Channels(client); }
