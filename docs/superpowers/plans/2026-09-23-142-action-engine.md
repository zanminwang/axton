...
- [ ] Remove the runtime/schema-validation assumption that every schema has at least one Model for valid ordinary-only Action schemas. Cover compiler generation and schema validation for a schema containing only an Action and no Models. Task 2 must prove real client open/submit for that schema once submission exists; an empty request models map alone does not establish this. Coordinate compiler schema validation order with the shared Action metadata rather than bypassing its type checks.
...
- [ ] Run cargo test -p axton-core --test contracts --locked; verify new tests fail at the missing Action contract.
...
- [ ] Add real SQLite tests that open a client on a truly Model-free Action schema and submit a scalar-only Action with no Model operations; failed argument validation leaving no queue/row; create+queue atomic rollback; reopening queued args/call ID unchanged; optional/list Model inputs, including all-absent optional operands and empty lists that still enqueue one Action; rejection/lifecycle dependency preservation. Inspect the physical queue with the existing harness.
- [ ] Run cargo test -p axton-sqlite --test actions --locked and demonstrate expected red failures.
...
- [ ] Generate a UUID once in Rust submit_action. Normalize args, derive Model operations/references/prerequisites from descriptors, then enqueue all state using the existing framework transaction. Allow zero inferred Model operations for any valid Action, including ordinary-only inputs, omitted optional Model operands and empty Model lists; retain rejection of arbitrary malformed legacy mutation input.
- [ ] Extend queue reads and freeze encoding to preserve identity/args. Model replay continues to use derived child operations. Never reconstruct ordinary args from those child rows. Frozen retries must produce byte-identical requests after reopen. Include reopening an older queued Action after regenerating a newer schema with retained Action/Model contracts: preserve the original version, call ID and canonical args rather than upgrading the pending intent to the latest version.
...
- [ ] Add axton_call with (owner_id,call_id) primary key, canonical request text and response text. Use INSERT ON CONFLICT DO NOTHING RETURNING followed by SELECT FOR UPDATE in the same transaction. The insert result supplies fresh, so an unexpectedly committed null-response row is treated as corrupt/incomplete storage rather than blindly executing again.
...
- [ ] Run cargo test -p axton-server --test host_contract --locked, then bash integration/persistence/server/run.sh after prerequisites. Verify real concurrent/rollback cases, inspect migration schema and commit.
...
- [ ] Cover an older retained Action result using Model read v1 while the current client requests authority in Model read v2. Select the appropriate versioned Loader contracts independently: the business result keeps v1 fields, and local authority carries the complete v2 state. Do not treat the v1 projection as current authority or report equal-stamp conflict merely because the two read contracts differ.
...
- [ ] Extend SyncCycle::complete and every native/SDK response consumer to carry completions separately from diagnostic reports. The current report.reports extraction must not drop completion events. Test a full durable pump emitting the correlated completion through the native boundary. Task 6 then proves that this same real pump resolves a real waiting ActionCall; fabricated observer events alone are insufficient for that later acceptance.
...
**Execution partition:** Implement the language runtimes first in two disjoint units: 6a owns shared JS/RN observers, raw local CRUD ports and lifecycle tests; 6b owns equivalent Dart runtime behavior and tests. Both expose internal `invokeAction(name, version, args, decode)` and `invokeDirectAction(name, version, args, decode)` ports, with an explicit decoder and SDK-owned Action types. Review these runtime units together, then implement 6c: compiler-generated client/backend bindings, retained codecs and executable generated integration. Within 6c, TS/RN plus backend bindings and Dart bindings may proceed in separate worktrees with separate executable fixtures; integrate both compiler changes before the combined review. Task 6 is complete only after all three units and their combined public-contract acceptance pass. This partition changes execution ownership, not the API or acceptance requirements below.
...
- [ ] Bind standalone client.models create/update/delete to a framework-owned local transaction, exposing get/query/watch alongside CRUD; tx.models keeps CRUD/read without watch. The existing concrete client has read/watch-only Model facades, so this requires real facade/port wiring rather than a type cast. Verify standalone local writes never enqueue and are observable through local reads/watch in TS/RN/Dart.
- [ ] Bind generated backend registration to a shared `ActionContext<Tx>` with `tx`, `userId`, `callId`, `changes` and `publish`. The runtime-facing generated `Handlers<Tx>` and existing `Loaders<Tx>` use the same application transaction type parameter; each Action handler receives `{ctx: ActionContext<Tx>, args: Input}`. Replace #141’s intentionally unconstrained type-only `Handlers<Ctx>` seam when the factory is connected, infer `Tx` from the application database, and typecheck trusted context members without erasing their public types.
- [ ] Wire generated Action namespace through concrete raw client submit/direct ports. Generated code converts DateTime/enums and Model values with existing serializers; Rust remains authoritative for validation and completion. Remove the compile-only limitation from public Action examples only when their runtime test passes. Shared `ActionCall`, `ActionOutcome`, `ActionStatus` and `ActionError` contracts must come from one SDK-owned runtime definition, re-exported by generated bindings; replace #141’s temporary generated declarations rather than casting nominally distinct Dart types. Keep generated Action-specific inputs, outputs and decoders separate, and verify assignment from real invocation results to the exported generic types in both languages.
