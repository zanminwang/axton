...
Value `type` and identity-field types reuse the existing structured Model descriptor grammar, such as `{"kind":"scalar","name":"string"}` or `{"kind":"enum","name":"Status"}`. Model/delete outputs carry `model`, while ordinary outputs carry `type`; only explicit handler-selected Model outputs need `handlerType` identity metadata. The descriptor is one contract for durable and direct delivery. #142/#116 may add execution-only fields, but must preserve these identity-source semantics. For a delete output use `kind: "deleteIdentity"` with `source.inputIdentity`; for plain values use `source: "handlerValue"`. Explicit Model handler values are typed identity objects (`TodoIdentity`), not bare key scalars or full Model rows.
...
Backend staging boundary: when a schema declares Actions, its generated backend artifact exports the Action `Handlers<Ctx>`, versioned input/output types and Loader contracts as contract-only declarations. Do not export a callable `createBackend` accepting those Action handlers until #142 supplies execution. Schemas containing only legacy mutations retain the existing executable backend factory and handler contracts. This prevents a cast into the old mutation runtime from advertising Action execution during #141.
