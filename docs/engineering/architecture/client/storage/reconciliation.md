...
Per model there are two tables with identical columns: the visible table named after the model and `axton_before_<Model>` for before images ([Writes](../engine/local-operations/writes.md)). Column types follow [Types](../../schema/types.md); the identity is the primary key in `@@id` order; each `@@unique` becomes a unique index on the visible table. Nine framework tables (`axton_schema`, `axton_client`, `axton_record`, `axton_subscription`, the four queue tables, `axton_rejection`) are created with `IF NOT EXISTS`; `axton_schema` holds one row, the canonical JSON of the descriptor and when it was written; `axton_client` carries `last_completed_push`, `push_models` and the `next_subscription` allocator ([Settlement](../engine/settlement.md)).
