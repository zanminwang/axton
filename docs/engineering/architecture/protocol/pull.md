...
- **What the client owes the page.** The wire says nothing about which task a page belongs to, so the client supplies that: a response is applied only while the subscription identity, the run and the committed progress it claims to continue are still the stored ones, and its authority and that progress commit in one local transaction. The delivery cursor is untouched by it, the records apply by stamp like any other authority, and the final page's `head` becomes the position ordinary delivery must reach before the load is complete ([Client Pull](../client/engine/pull.md), [Typed client](../sdks/typed-api/client.md)).
