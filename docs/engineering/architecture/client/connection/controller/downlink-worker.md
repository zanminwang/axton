...
| `changed {scopes}` | A commit moved these Scopes' cursors, or committed their first delivery boundary. Nothing to execute yet; [#150](https://github.com/zanminwang/axton/issues/150) Task 3 turns it into a subscription status update. |
...
3. The first frame must be an acknowledgement for exactly the requested channels. Its heads are committed as the first boundary of every Scope that had none, and that commit is announced with `changed`. If every head then equals the durable cursor, the session is streaming at once; otherwise a `request` starts the catch-up and repeats while any channel continues. Frames that arrive meanwhile wait in the queue.
