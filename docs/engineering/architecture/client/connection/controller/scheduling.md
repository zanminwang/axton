...
`pause` aborts the lane's in-flight request, tells Rust to pause, waits for the running body to finish, and wakes the loop so it observes the paused state; a failure caused by the abort is reported as success so no backoff is scheduled. On the downlink lane the host abandons the session's socket and request - and the historical page in flight, which belongs to the lane rather than to the socket - and Rust ends the session without backoff; `resume` clears the deferral, so the page is asked for again. `resume` clears the pause and marks the lane dirty. `close` stops the loop, aborts requests and detaches; controls on a closed connection are no-ops, so a stale handle cannot affect a replacement.
