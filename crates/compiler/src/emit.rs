...
pub fn client_typescript(v: &Value, runtime: &str) -> String {
    ...
    writeln!(o, "export {{ ActionError, type ActionCall, type ActionOptions, type ActionOutcome, type ActionStatus, type ServerOptions, type Connection, type ConnectionOptions, type Subscription, type SubscriptionStatus }} from \"{runtime}\";").unwrap();
    ...
    writeln!(o, " private constructor(client: Client, connection: Connection | undefined) {{ this.client = client; this.connection = connection; this.models = liveModels(client); this.actions = makeActions(client); {}this.scopes = new Scopes(client); this.channels = new Channels(client); }}", if has_mutations { "this.mutate = new Mutate(client); " } else { "" }).unwrap();
    if has_mutations {
