...
| `backend.json` | server config: schema, retained mutation versions with input snapshots and `knownFields`, `models` (every retained model read contract: `{name, version, identity, fields, enums}`), loaders (model names), inverses, unique constraints | [Server / Backend interface](../server/backend-interface.md) |
...
The CLI writes both histories beside the schema, at `history/mutations.json` and `history/models.json` in the input directory; `--mutation-history FILE` and `--model-history FILE` override those locations. Commit history to Git so subsequent compilation can retain old contracts. Every output is staged as a temporary file and moved into place only after all of them were written. When the default is absent and a history exists at the superseded `OUTPUT_DIR/mutation-history.json`, the CLI reads it, writes the new location, leaves the old file in place and reports the move on stderr; it never reinitializes a history silently.
