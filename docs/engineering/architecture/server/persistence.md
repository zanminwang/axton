...
| `scan {channel, after, limit}` | invalidation rows after a cursor whose record is still a member of the channel (filtered before the limit), in order, each joined with the record's current stamp from `axton_record`; a row whose record has no stamp is a storage defect |
...
| `lockRecord {model, identityKey}` | write-lock an existing record row with a no-op `UPDATE … SET stamp=stamp` and return its unchanged stamp; `null`, creating nothing, when the record has no row |
| `memberships {model, identityKey}` | the Channels the record is a persistent member of, each once, in the database's order (the engine sorts them) |
| `setMembership {channel, model, identityKey, present}` | add the membership (creating the Channel at head 0; the record row must exist) or remove it; idempotent both ways and never allocates a cursor |
| `savepoint`, `rollback`, `release {ordinal}` | per-mutation savepoints |

Tables: `axton_client`, `axton_call`, `axton_channel`, `axton_record`, `axton_invalidation`, `axton_membership` ([migration.sql](../../../../packages/postgres/migration.sql)). `axton_membership` is keyed record-first `(model, identity_key, channel)` for a record's memberships, with a Channel-first index; it is separate from invalidation rows. The shared settlement issues `lockRecord`, `memberships` and `setMembership` for every record a settlement changes or enrolls ([Publish](engine/publish.md#5-building-block-view)). Membership is never derived from invalidation rows: the migration creates an empty table, so on an existing development database the application re-enrolls its records explicitly ([#140](https://github.com/zanminwang/axton/issues/140)).
...
Properties the SQL holds: `claim` uses `SELECT … FOR UPDATE`, so retries of one client serialize on the row; `claimCall` uses `INSERT … ON CONFLICT DO NOTHING RETURNING` followed by `SELECT … FOR UPDATE` in the supplied transaction, so a duplicate sees the original request and response after the first transaction commits. A committed null response is a storage fault. `saveCall` checks the full creating transaction ID (`claim_tx`) and refuses overwrites. No call responses are automatically pruned. `advanceStamp` and `ensureStamp` allocate stamps with atomic upserts that return the new value, so concurrent first publications agree on 1 and an established stamp is never overwritten; `publish` allocates the cursor the same way and refuses a stamp that is not the record's current one; `scan` joins `axton_membership` before `ORDER BY … LIMIT`, so a removed record's retained row is skipped rather than filling a page, and is a `LEFT JOIN` from the invalidation row to the stamp row, so a page carries the stamp of the content the loader reads, not the stamp the record had when it was last published. `lockRecord` is an `UPDATE`, not `SELECT … FOR UPDATE`: under Repeatable Read, a row a concurrent committed transaction only locked can still be written from an older snapshot, while a row it updated makes that write fail serialization, so the runner retries it. Counters are `bigint` with safe-range checks and are narrowed to safe integers on the way out; a driver whose tool cannot send a bigint parameter sends it as text.
...
- **A stamp advances without a channel; one push publishing to two channels carries one stamp to both and advances each head once; an external write advances the stamp on every call while a push publishing an unchanged record does not; concurrent first publications initialize one stamp of 1; a rolled-back transaction removes a first initialization with its publication; `publish` refuses a missing or stale stamp; `scan` pairs the cursor with the current stamp** (guarantee D3). Evidence: `advanceStamp increments without a channel: no invalidation, no channel head`, `one push enrolled in two channels carries the same stamp to both and advances each head once`, `an external write advances the stamp on every call; a push re-enrolling an unchanged member neither advances nor publishes it`, `concurrent first publications initialise one stamp of 1 and never overwrite an established one`, `a rolled-back transaction removes a first initialisation together with its publication`, `publish refuses a record without metadata or with a stamp that is not its current one`, `scan pairs the invalidation cursor with the current record stamp; a missing record row is a storage defect`, `concurrent notifies of one record receive distinct stamps`.
- **Membership is persistent, idempotent and separate from publication; a record guard makes a stale Repeatable Read writer retry.** `setMembership` needs the record row, creates the Channel at head 0 and never moves a head; duplicate adds and removes are no-ops; memberships survive into a new transaction and roll back with a savepoint or transaction; `lockRecord` answers `null` without creating a row and preserves the stamp. A writer whose snapshot predates a committed membership-only transaction (`lockRecord` then `setMembership`) fails serialization when it writes the record and its retry reads the new membership; without the guard it commits on the stale view. Evidence: [driver-conformance.test.mjs](../../../../integration/persistence/server/driver-conformance.test.mjs) `membership adds, lists and removes idempotently, needs record metadata and never allocates a cursor`, `savepoint and transaction rollback restore membership relationships`, `a membership-only writer's no-op record UPDATE makes a stale-snapshot writer of the same record retry`, run against `pg`, `prisma` and `drizzle`.
- **Membership filtering, removal and concurrent declarations hold on a real database.** A removed record is skipped before the scan limit in delta and Bootstrap pages; re-adding publishes current state at a fresh position without a new stamp; a deleted member yields `null` and its recreation is distributed again; removal fabricates no deletion and keeps business rows, stamps, heads and retained rows. Touch/add and touch/remove race in both commit orders with the final stamp on the enrolled Channel and nothing on the removed one; membership-only races keep the stamp and a duplicate add publishes once; a call rejected after settlement and a failed transaction roll back relationships, stamps, heads and wakes, and the retried batch settles once. Evidence: [membership.test.mjs](../../../../integration/persistence/server/membership.test.mjs), every test in the file, over the `pg` shim.
...
Executed 2026-09-26 on this branch with PostgreSQL 14.23: `bash integration/persistence/server/run.sh` (effects, driver-conformance, runtime and host-contract, actions and membership suites all passing).
