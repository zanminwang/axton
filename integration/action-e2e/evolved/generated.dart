...
class TodoCreate implements TodoCreateInput {
 ...
 final String title;
 final String? note;
 const TodoCreate({required this.id,required this.title,required this.note});
 @override
...
class Mutations {
 ...
 Future<Call<AddTodoOutput>> addTodo({required TodoCreateInput todo, AddTodoStore? store}) => client.invokeAction<AddTodoOutput>('AddTodo', 2, {'todo': _dartActionEncode(todo)}, (value) { final row = (value as Map).cast<String,dynamic>(); return AddTodoOutput(todo: Todo.fromRecord((row['todo'] as Map).cast<String,dynamic>())); }, store: store);
...
class DirectMutations {
 ...
 Future<AddTodoOutput> addTodo({required TodoCreateInput todo, AddTodoStore? store}) => client.invokeDirectAction<AddTodoOutput>('AddTodo', 2, {'todo': _dartActionEncode(todo)}, (value) { final row = (value as Map).cast<String,dynamic>(); return AddTodoOutput(todo: Todo.fromRecord((row['todo'] as Map).cast<String,dynamic>())); }, store: store);
...
class Queries {
 ...
 Future<SearchTodosOutput> searchTodos({required String? query, SearchTodosStore? store}) => client.invokeDirectAction<SearchTodosOutput>('SearchTodos', 3, {'query': _dartActionEncode(query)}, (value) { final row = (value as Map).cast<String,dynamic>(); return SearchTodosOutput(todos: (row['todos'] as List).map((e) => Todo.fromRecord((e as Map).cast<String,dynamic>())).toList(), first: row['first'] == null ? null : Todo.fromRecord((row['first'] as Map).cast<String,dynamic>()), count: row['count'] as int, labels: (row['labels'] as List).map((e) => e as String).toList(), hint: row['hint'] == null ? null : row['hint'] as String); }, store: store);
}
