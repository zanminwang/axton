...
pub enum ActionInput { Value { name: String, ty: FieldType, cardinality: Cardinality },
    Model { slot: Slot } }
...
Use `Cardinality::Single/Optional/List` for Action outputs, with List always non-null and non-null elements. `ActionInput::Value` must distinguish absent optional values from explicit null using the compiler's existing presence rules. If an existing `FieldDecl` cannot carry Action output list/nullability without accepting unsupported persisted Model shapes, use the same fields in a separate `ActionFieldDecl`; do not weaken Model field validation.
...
{"name":"AddTodo","version":1,
 ...
 "outputs":[{"name":"todo","kind":"model","model":"Todo","cardinality":"single","source":{"inputIdentity":"todo"}},
            {"name":"relatedTodo","kind":"model","model":"Todo","cardinality":"optional","source":"handlerIdentity"}]}
```
...
- [ ] Add red tests for ordinary scalar/enum inputs; Model create/update/delete single, optional and list; restrictions/bindings/prerequisites/sequence; required/nullable/list scalar, enum and Model outputs; output omission; duplicate implicit/explicit names; `action Call(...)` collision with reserved `call`; unsupported nested/nullable lists and object shapes. Assert diagnostics identify the source location and offending name.
...
- [ ] Assert absent optional input maps to null and list descriptors retain source input order/identity position rather than a deduplicated set. Re-run compiler tests and commit.
...
- [ ] Add descriptor tests that distinguish Action ordinary inputs from Model operands, include explicit output kind/cardinality/identity source and implicit input binding, preserve version/restriction/binding/prerequisite/sequence metadata, and provide one Action record for both delivery modes. Test an empty list, absent optional, duplicate identities and void output using descriptor assertions. For explicit `relatedTodo Todo?`, assert the handler-side descriptor requires a key-field identity object (`{id: String}` for Todo), while the client-side result is `Todo | null`; composite keys require an object of all key fields, and Model lists keep identity-object order.
- [ ] Run focused compiler/CLI/history tests to establish red.
- [ ] Add a new `actions` descriptor section for #142/#116, with input/output metadata and explicit identity source. Keep existing `clientPolicies` valid only for legacy executable mutation fixtures; do not fabricate policies for ordinary Action inputs. Preserve the current runtime fixture through an internal compatibility descriptor/adapter until #142 consumes Actions. Document the exact temporary descriptor mapping in `docs/engineering/architecture/compiler/generate.md`.
- [ ] Re-run `cargo test -p ahead-compiler --locked`. Inspect `history.rs`/CLI version fencing so it compares Action input contracts and does not silently drop retained Model mutation features. Commit.
...
- [ ] Emit a shared generic TypeScript handle/outcome and Action-specific input/output interfaces; emit an `ActionClientContract` whose `actions.foo` and `actions.call.foo` share the same argument/result type and a backend handler contract whose explicit Model outputs are typed identity objects. For example, the handler return type for `relatedTodo Todo?` is `{ relatedTodo: TodoIdentity | null }`, where a single-key Todo identity is `{ id: string }`; reject a bare `string` and a full Todo, while the client result remains `{ relatedTodo: Todo | null }`. Keep concrete client wiring on the existing path until #142. Ensure generated `client.models` and `tx.models` reflect #145 local CRUD/read/watch split, with no transactional watch.
...
- [ ] Add Dart analyzer-positive fixtures for value/Model/nullable/list/void outputs, including a generated `TodoIdentity` class in `{relatedTodo: TodoIdentity?}` handler output and `Todo?` client output, flattened Model operands, local Model and transaction interfaces, default handle and direct final-output signatures. Add negative analyzer cases for `tx.actions`, duplicate/invalid shapes, and absent framework handle fields.
