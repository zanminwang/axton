...
| `memberships {model, identityKey}` | the Channels the record is a persistent member of, sorted and unique |
...
Tables: `axton_client`, `axton_call`, `axton_channel`, `axton_record`, `axton_invalidation`, `axton_membership` ([migration.sql](../../../../packages/postgres/migration.sql)). `axton_membership` is keyed record-first `(model, identity_key, channel)` for a record's memberships, with a Channel-first index; it is separate from invalidation rows. The settlement engine does not issue the membership operations yet ([#140](https://github.com/zanminwang/axton/issues/140)).
