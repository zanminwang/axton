...
**Execution partition:** Implement the language runtimes first in two disjoint units: 6a owns shared JS/RN observers, raw local CRUD ports and lifecycle tests; 6b owns equivalent Dart runtime behavior and tests. Both expose internal `invokeAction(name, version, args, decode)` and `invokeDirectAction(name, version, args, decode)` ports, with an explicit decoder and SDK-owned Action types. Review these runtime units together, then implement 6c: compiler-generated client/backend bindings, retained codecs and executable generated integration. Task 6 is complete only after all three units and their combined public-contract acceptance pass. This partition changes execution ownership, not the API or acceptance requirements below.
