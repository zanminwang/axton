    ...
    for (name, version) in &latest {
        ...
        if keys.is_empty() {
            ...
            let union = keys
                ...
                .collect::<Vec<_>>()
                .join("|");
            writeln!(o, "export type {name}Options = ActionOptions<{union}>;").unwrap();
        }
    }
    o.push_str("export function makeActions(port:ActionPort) { return {\n");
    for (name, version) in &latest {
        writeln!(o, " {}: (args:{name}Input, options?:{name}Options):Promise<ActionCall<{name}Output>> => port.invokeAction('{name}',{version},encode{name}Input(args),decode{name}Output,options),", lower(name)).unwrap();
    }
    o.push_str(" call: {\n");
    for (name, version) in &latest {
        writeln!(o, "  {}: (args:{name}Input, options?:{name}Options):Promise<{name}Output> => port.invokeDirectAction('{name}',{version},encode{name}Input(args),decode{name}Output,options),", lower(name)).unwrap();
    }
    o.push_str(" }\n}; }\n");
}
...
pub fn client_typescript(v: &Value, runtime: &str) -> String {
    ...
    writeln!(o, "export {{ ActionError, type ActionCall, type ActionOptions, type ActionOutcome, type ActionStatus, type ServerOptions, type Connection, type ConnectionOptions }} from \"{runtime}\";").unwrap();
    let has_mutations = !arr(v, "mutations").is_empty();
    let imports = if has_mutations {
        "schema, liveModels, makeActions, GeneratedTransaction, Mutate, type LiveModels"
    } else {
        "schema, liveModels, makeActions, GeneratedTransaction, type LiveModels"
    };
    ...
    writeln!(o, " private constructor(client: Client, connection: Connection | undefined) {{ this.client = client; this.connection = connection; this.models = liveModels(client); this.actions = makeActions(client); {}this.channels = new Channels(client); }}", if has_mutations { "this.mutate = new Mutate(client); " } else { "" }).unwrap();
    if has_mutations {
...
pub fn backend_typescript(v: &Value, runtime: &str) -> String {
    ...
    if arr(v, "mutations").is_empty() {
        writeln!(o, "export {{ ActionRejected, devAuth, type ActionContext, type HandlerCall, type LoaderCall, type RecordRef }} from \"{runtime}\";").unwrap();
    } else {
...
fn dart_action_runtime(v: &Value, o: &mut String) {
    ...
    for direct in [false, true] {
        ...
        for (name, version) in &latest {
            ...
            writeln!(o, " Future<{}> {}({}) => client.{}<{output}>('{name}', {version}, {{{arguments}}}, {decoder}, store: {store});", if direct { output.clone() } else { format!("ActionCall<{output}>") }, lower(name), signature, if direct { "invokeDirectAction" } else { "invokeAction" }).unwrap();
        }
    }
    o.push_str("}\n");
}
